...
import org.rg.site.cms.service.ArticleService;
...
public class UploadController {
			...
			if(StringUtils.isNotBlank(suffix)){
				if(suffix.equalsIgnoreCase(".xls") || suffix.equalsIgnoreCase(".xlsx") || suffix.equalsIgnoreCase(".txt")|| suffix.equalsIgnoreCase(".png")
					...
					if(StringUtils.isNotBlank(preFileName)){
						preFileName = realPath + preFileName.substring(8);
						dropFile(preFileName);
					}
					String id = request.getParameter("articleId");
					updateImageUrl(id, result);
				}else{
	...
	private void updateImageUrl(String id, String coverImageUrl){
	...
	private boolean dropFile(String fileName){
